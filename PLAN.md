...
| 6 | **§9 Q5 — validated lesson content** — **A1 DONE FOR ALL THREE PAIRS (August 2026).** Maps: `content/curriculum/{es-PY-en-speaker,en-es-speaker,es-PY-sv-speaker}.md`. Content: 48 lessons, all schema-valid — `es-PY>en-speaker` A1+A2 (positions 1–24), `en>es-speaker` A1 (1–12), `es-PY>sv-speaker` A1 (1–12). Passes 2–3 run by Sonnet 5 in-repo against `lessonImportItemSchema` via `npm run lessons:validate`, not by eye. Both `*.sample.json` placeholders deleted. **Still open:** import through `/admin` (needs Phase 0); A2+ for the two newer pairs — and for `es-PY>sv-speaker` that is *blocked on an owner answer*, see §9 Q12; B1/B2 for all three, which §9 Q4 says should wait until A1/A2 have been used for real. | Owner (+ any model to import) | — |
