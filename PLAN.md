...
| 2 | **Live verification session**: run the Phase 2/3/4/4B/5/5B/6/7 acceptance checklists on both real phones, fix the fallout. Expect real breakage, not a formality — budget a full session. **Narrowed (August 2026):** the offline half is now automated — `npm test` (99 unit tests over the scheduler, streak/XP, caps, tier gate, retry clamp, audio-cache key, dictionary parity and the request schemas) plus lint/typecheck/`lessons:validate`, all four in CI on every push. What is left for this item is what genuinely needs a phone and a key: audio capture on both browsers, real model output, real latency, real quota. | **Opus 5** | 1 |
...
| 6 | **§9 Q5 — validated lesson content** — **A1 DONE FOR ALL THREE PAIRS (August 2026).** Maps: `content/curriculum/{es-PY-en-speaker,en-es-speaker,es-PY-sv-speaker}.md`. Content: 60 lessons, all schema-valid — `es-PY>en-speaker` A1+A2 (positions 1–24), `en>es-speaker` A1+A2 (1–24), `es-PY>sv-speaker` A1 (1–12). Passes 2–3 run by Sonnet 5 in-repo against `lessonImportItemSchema` via `npm run lessons:validate`, not by eye. Both `*.sample.json` placeholders deleted. **Still open:** import through `/admin` (needs Phase 0); A2 for `es-PY>sv-speaker`, which is *blocked on an owner answer*, see §9 Q12; B1/B2 for all three, which §9 Q4 says should wait until A1/A2 have been used for real. | Owner (+ any model to import) | — |
...
| Offline test suite + CI | `tests/*.test.ts` (`node:test` via `tsx`, no new dependency, no database, no keys) and `.github/workflows/ci.yml` running lint → typecheck → `lessons:validate` → `npm test` on every push and PR. Covers the logic that runs unattended and fails *silently*: SM-2-lite intervals/ease floor/cap (§13.3), the XP-and-streak decision incl. the one-shield-per-ISO-week rule and the user-timezone rule (§12.2), the monthly TTS char stop (§16 defect 2), the tier gate (§15.3), the retry-hint clamp (§6.4), the listening-audio cache key, three-locale dictionary key/arity parity, and the §6.3 request bounds. Also the seed-data ↔ prompt-assembler contract (§10.3): the `patternKey` taxonomy lives in data and is substituted by code, and a template that spells `{{error_taxonomy}}` differently ships the literal placeholder to the model with no symptom but a slowly rotting dashboard — asserted in both directions, all three pairs, both modes. That test is why `scripts/seedPairs.ts` exists: `seed.ts` runs `main()` on import, so its data half had to move where a test can read it without a database. Each assertion was checked against a deliberate mutation of the code it covers, so the suite is known to fail when the behaviour changes rather than merely to pass. DB-backed halves are deliberately **not** mocked — a Drizzle mock would test the mock; those stay Phase 0 work |
