...
| Offline test suite + CI | `tests/*.test.ts` (`node:test` via `tsx`, no new dependency, no database, no keys) and `.github/workflows/ci.yml` running lint → typecheck → `lessons:validate` → `npm test` on every push and PR. Covers the logic that runs unattended and fails *silently*: SM-2-lite intervals/ease floor/cap (§13.3), the XP-and-streak decision incl. the one-shield-per-ISO-week rule and the user-timezone rule (§12.2), the monthly TTS char stop (§16 defect 2), the tier gate (§15.3), the retry-hint clamp (§6.4), the listening-audio cache key, three-locale dictionary key/arity parity, and the §6.3 request bounds. Each assertion was checked against a deliberate mutation of the code it covers, so the suite is known to fail when the behaviour changes rather than merely to pass. DB-backed halves are deliberately **not** mocked — a Drizzle mock would test the mock; those stay Phase 0 work |
